...
/*-----------------------------------------------------------------------
 * LoadChemCkptB: distribute chem checkpoint to virtual filesystem jjb
 *-----------------------------------------------------------------------*/
    ...
    ForSubgridI(s_i, all_subgrids)
    {
      ...
      if (process == p)
      {
        ...
        if (!process)
        {
          ...
          tools_WriteInt(file, &num_procs, 1);
          tools_WriteInt(file, &num_vars, 1);

          file_pos += 6 * tools_SizeofDouble + 5 * tools_SizeofInt;
        }
        ...
        ptr = DataboxChemCkptCoeff(databox, ix, iy, iz, num_vars);
        ...
        for (k = 0; k < nz; k++)
        {
          for (j = 0; j < ny; j++)
          {
            for (i = 0; i < nx; i++)
            {
              tools_WriteDouble(file, ptr + i * num_vars + j * (NX) * num_vars + k * (NX)*(NY) * num_vars, (num_vars));
            }
          }
        }

        file_pos += 9 * tools_SizeofInt + (nx * ny * nz * num_vars) * tools_SizeofDouble;
